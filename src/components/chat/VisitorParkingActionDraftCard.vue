...
const canConfirm = computed(() => {
  ...
  return Boolean(
    form.parkingLotId && form.visitorName.trim() && form.carNumber.trim()
...
<template>
  <section v-if="showInlineTrigger && !hiddenAfterCompletion" class="parking-draft-action">
    <button
      type="button"
    ...
    <div v-if="isOpen" class="parking-modal-backdrop" @mousedown.self="closeForm">
      <section class="parking-modal" role="dialog" aria-modal="true" aria-labelledby="parking-action-title">
        ...
        <div class="parking-form">
          ...
          <label>
            <span>방문객 이름</span>
            <input v-model="form.visitorName" type="text" maxlength="50" :readonly="isCancel" placeholder="방문객 이름" />
          </label>
...
.parking-form input, .parking-form select { width: 100%; min-height: 38px; box-sizing: border-box; padding: 8px 10px; border: 1px solid var(--color-border); border-radius: 6px; background: var(--color-surface); color: var(--color-text); font: inherit; font-weight: 500; }
